...
def seeing_post(db: Session, post_id: str, user_id: str) -> Seen:
	...
	if db.query(models.Seen).filter(models.Seen.post_id == post_id, models.Seen.user_id == user_id).count():
		raise HTTPException(400, 'Already favorite the post')

	favorite_orm = models.Seen(user_id=user_id, post_id=post_id)
